...
class TestDB:
    ...
    def test_dict_cursor(self, mocker: MockerFixture) -> None:
        ...
        m_cw.assert_called_once_with(conn_obj.cursor.return_value, timeout=None)
    ...
    def test_dict_cursor_tout(self, mocker: MockerFixture) -> None:
        ...
        m_cw.assert_called_once_with(conn_obj.cursor.return_value, timeout=99.1)
...
class TestCursorWrapper:
    ...
    def mock_cur(self, mocker: MockerFixture) -> None:
        self.m_cur = mocker.create_autospec(Cursor, spec_set=True)
    ...
    def test_init(self) -> None:
        c = db.CursorWrapper(self.m_cur)
        assert c._cur == self.m_cur
        assert c._def_tout is None

    def test_init_tout(self) -> None:
        c = db.CursorWrapper(self.m_cur, timeout=99.1)
        assert c._cur == self.m_cur
        assert c._def_tout == 99.1

    def test_execute(self, mock_add_timeout: Any) -> None:
        c = db.CursorWrapper(self.m_cur)
    ...
    def test_execute_full(self, mock_add_timeout: Any) -> None:
        c = db.CursorWrapper(self.m_cur)
    ...
    def test_executemany(self, mock_add_timeout: Any) -> None:
        c = db.CursorWrapper(self.m_cur)
        ret = c.executemany("query1")
        assert ret == self.m_cur.executemany.return_value
        mock_add_timeout.assert_called_once_with("query1", None)
        self.m_cur.executemany.assert_called_once_with(
            mock_add_timeout.return_value,
            args=(),
        )

    def test_executemany_full(self, mock_add_timeout: Any) -> None:
        c = db.CursorWrapper(self.m_cur)
        ret = c.executemany("query1", (("arg1", "arg2"), ("arg3", "arg4")), timeout=99.1)
        assert ret == self.m_cur.executemany.return_value
        mock_add_timeout.assert_called_once_with("query1", 99.1)
        self.m_cur.executemany.assert_called_once_with(
            mock_add_timeout.return_value,
    ...
    def test__add_timeout(
        ...
        c = db.CursorWrapper(self.m_cur, timeout=def_tout)
